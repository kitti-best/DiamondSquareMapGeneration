...
class DiamondSquareMapWithBiome:
    ...
    def __perform_square(self, step):
        ...
        for y in range(step, self.h, step):
            for x in range(step, self.w, step):
                ...
                avg_x = (x1 + x2 + x3 + x4) // 4
                avg_y = (y1 + y2 + y3 + y4) // 4
                if not self.__map_list[avg_y][avg_x]:
    ...
    def __perform_corner(self):
        ...
        self.__map_list[0][0] = min_range
        self.__map_list[0][self.w - 1] = max_range
        self.__map_list[self.h - 1][0] = min_range
        self.__map_list[self.h - 1][self.w - 1] = max_range
    ...
    def __create_biome(self):
        cell_val = self.corner_strength[0]
        tl_x, tl_y = (random.randint(1, self.w - 7), random.randint(1, self.h - 7))
        print(tl_x, tl_y)
        for off_y in range(6):
            for off_x in range(6):
                x = tl_x + off_x
                y = tl_y + off_y
                self.__map_list[y][x] = cell_val
